...
public class Classroom {
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        SNode iterate = musicalChairs, loser = null;
        //find node before loser (if index = 0, node before is musicalChairs)
        for(int i = 0; i < index; i++){
            iterate = iterate.getNext();
        }
        loser = iterate.getNext();
        //if loser is the last node in musicalChairs, redefine musicalChairs as the one before it
        if(loser == musicalChairs){
            musicalChairs = iterate;
            musicalChairs.setNext(loser.getNext());
        }else{ //remove loser from musicalChairs
            iterate.setNext(loser.getNext());
        }
        //insert loser into studentsInLine
        insertByHeight(loser.getStudent());
    }
    ...
    public void insertByHeight(Student studentToInsert) {
        SNode enter = new SNode();
        enter.setStudent(studentToInsert);
        if(studentsInLine == null){
            studentsInLine = enter;
            studentsInLine.setNext(null);
        }else if(studentsInLine.getNext() == null){
            if(studentsInLine.getStudent().getHeight() < enter.getStudent().getHeight()){ // add new student to back of line
                studentsInLine.setNext(enter);
            }else{
                enter.setNext(studentsInLine);
                studentsInLine = enter;
            }
        }else{
            //find place for new student
            SNode iterate = studentsInLine;
            while(iterate.getNext() != null && iterate.getNext().getStudent().getHeight() <= enter.getStudent().getHeight()){
                iterate = iterate.getNext();
            }
            if(iterate.getStudent().getHeight() > enter.getStudent().getHeight()){ //if student is shorter than first student -> add to front
                enter.setNext(studentsInLine);
                studentsInLine = enter;
            }else if(iterate.getNext() == null){ //if student is taller than last student -> add to back
                iterate.setNext(enter);
            }else{ //add new student between student they're same/greater height than and next
                enter.setNext(iterate.getNext());
                iterate.setNext(enter);
            }
        }
    }    
    ...
    public void eliminateLosingStudents() {
        int remaining = 0;
        SNode iterate = musicalChairs;
        //determine number of remaining students
        do{
            iterate = iterate.getNext();
            remaining++;
        }while(iterate != musicalChairs);
        //remove the students until there is only one remaining
        while(remaining > 1){
            moveStudentFromChairsToLine(remaining);
            remaining--;
        }
